...
class Controller:
    ...
    def changesystembrightness():
        ...
        brightness = sbcontrol.get_brightness(display=0)[0]
        if brightness is None:
            brightness = 100
        currentBrightnessLv = float(brightness)/100
        currentBrightnessLv += int(Controller.pinchlv)/50
        print(currentBrightnessLv)
        if currentBrightnessLv > 1.0:
            currentBrightnessLv = 1.0
        elif currentBrightnessLv < 0.0:
            currentBrightnessLv = 0.0       
        sbcontrol.fade_brightness(int(100*currentBrightnessLv) , start = brightness)
...
class GestureController:
    ...
    ----------
    gc_mode : int
        indicates whether gesturease is running or not,
            ...
            while GestureController.cap.isOpened() and GestureController.gc_mode:
                ...
                if results.multi_hand_landmarks:                   
                    ...
                    for hand_landmarks in results.multi_hand_landmarks:
                        mp_drawing.draw_landmarks(image, hand_landmarks, mp_hands.HAND_CONNECTIONS,  mp_drawing.DrawingSpec(color=(211,0,148), thickness=3))
                else:
                    Controller.prev_hand = None
                cv2.imshow('GesturEase', image)
